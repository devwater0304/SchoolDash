...
void main() {
  testWidgets('shows today lunch before classes', (tester) async {
    ...
    expect(find.text('6월 16일 화요일'), findsOneWidget);
  });
  ...
  testWidgets('expands an upcoming meal without replacing today main meal', (
    tester,
  ) async {
    await _pumpMealScreen(tester, DateTime(2026, 6, 15, 9));

    await tester.tap(find.text('6월 16일 화요일'));
    await tester.pumpAndSettle();
    ...
    await tester.tap(find.text('6월 16일 화요일'));
    await tester.pumpAndSettle();
    expect(find.text('• 카레라이스'), findsNothing);

    await tester.tap(find.text('6월 17일 수요일'));
    await tester.pumpAndSettle();
