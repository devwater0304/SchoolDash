...
NEIS 응답에 오늘 시간표가 없으면 학사일정을 함께 확인합니다. 방학·공휴일·휴업일이 확인되면 “오늘은 쉬는 날!”과 일정명을, 확인되지 않으면 “오늘은 시간표가 없어요”를 표시합니다. 네트워크나 API 문제가 생기면 Home을 막지 않고 기존 샘플 시간표를 임시로 보여줍니다.

## NEIS 학사일정

`SchoolSchedule` API는 교육청 코드와 학교 코드, 날짜 범위를 사용합니다. NEIS의 학사일자, 행사명, 수업공제일명, 학년별 행사 여부는 `SchoolEvent`로 변환되고 `SchoolCalendarService`가 방학·공휴일·휴업일만 비수업일로 판단합니다. 알 수 없는 행사는 수업일로 유지합니다.

같은 날짜의 일정 결과는 앱 실행 중 메모리에 재사용합니다. 일정 조회가 실패하면 휴일로 추정하지 않고 일반 수업일로 처리해, 시간표 요청과 Home 표시가 멈추지 않습니다.
